...
pub fn get_rename_callback(
    ...
    Callback::from(move |e: MouseEvent| {
        ...
        display.set(html!(
            <input
                onblur={onblur}
                onkeypress={get_kayboard_handler(state, dispatch, title.clone(), display.clone(), input_ref.clone(), kind, chapter_index)}
                ref={input_ref.clone()}
...
fn get_kayboard_handler(
    state: Rc<State>,
    dispatch: Dispatch<State>,
    title: UseStateHandle<String>,
    name_display: UseStateHandle<VNode>,
    input_ref: NodeRef,
    kind: RenameKind,
    chapter_index: Option<usize>,
            ...
            if let Some(input) = input_ref.cast::<HtmlInputElement>() {
                ...
                spawn_local(async move {
                    ...
                    if result {
                        ...
                        let mut temp_project = state.project.clone().as_ref().unwrap().clone();
                        match kind {
                            ...
                            RenameKind::Note => {
                                for note in &mut temp_project.chapters[chapter_index.unwrap()]
                                    .notes
                                    .iter_mut()
                                {
                                    if *note == *title {
